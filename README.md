...
| **FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness** | FlashAttention, an IO-aware exact attention algorithm that uses tiling to reduce the number of memory reads/writes between GPU high bandwidth memory (HBM) and GPU on-chip SRAM. | [Read Paper](https://arxiv.org/pdf/2205.14135) |
